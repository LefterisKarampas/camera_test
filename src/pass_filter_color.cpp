...
void cloud_cb (const pcl::PCLPointCloud2ConstPtr& input){
	...
	if(flag){
		...
		for(int i =0;i<size;i++){
			try{
				listener.waitForTransform(input->header.frame_id,color, ros::Time(0),ros::Duration(3.0));
				listener.lookupTransform(input->header.frame_id,color,ros::Time(), transform);
				// Y 
...
{
	// Initialize ROS
	ros::init (argc, argv, "pass_filter_color");
	ros::NodeHandle nh;
	// Create a ROS publisher for the output point cloud
	color = argv[1];
	ros::Subscriber sub = nh.subscribe ("colors", 1, frames);
	// Spin
	ros::spin ();
} 
